...
async def edit_project(project: schemas.Project) -> Union[HTTPException, Response]:
    try:
        _project = await models.Project.objects.get(id=project.id)
    except:
        raise HTTPException(status_code=404, detail="Проект с данным идентификатором не найден.")
...
async def delete_project(project_id: int) -> Union[HTTPException, Response]:
    try:
        _project = await models.Project.objects.get(id=project_id)
    except:
        raise HTTPException(status_code=404, detail="Проект с данным идентификатором не найден.")
